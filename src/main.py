...
def trancar(update, context):
    if random.random() < 0.79:
        ...
        lista = ["Enviando emojis ofensivos para a grad...", "Passando trote para a PPUSP...",
        ...
        while True:
            if random.random() < 0.55:
                if len(lista) > 0:
                    ...
                    context.bot.send_message(chat_id=update.effective_chat.id, text=txt)
                    context.bot.ban_chat_member(chat_id=update.effective_chat.id, user_id=update.effective_message.from_user.id, until_date=date_unban)
                    return
            else:
